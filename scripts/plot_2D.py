...
class ObstacleAvoidanceScan:
    def __init__(self):
        ...
        self.max_obstacle_distance = float(rospy.get_param('~max_obstacle_distance', None))    
        self.density_gain = float(rospy.get_param('~density_gain', None))
        self.observation_radius = float(rospy.get_param('~observation_radius', None))
        self.min_threshold = float(rospy.get_param('~min_threshold', None))
        self.cumulative_distance = float(rospy.get_param('~cumulative_distance', None))
        ...
        # Position the window at (x, y) coordinates on the screen
        x, y = 1000, 100
    ...
    def scan_callback(self, scan_data):
        ...
        self.num_laser_points = num_points = len(scan_data.ranges)
    ...
    def calculate_potential(self, robot_points, obstacle_points, clusters, obstacle_idx, cumulative_distance, density_gain):
        ...
        for cluster, points in cluster_dict.items():
            ...
            self.ax.scatter(points[closest_index][0], points[closest_index][1], c='g', s=10)
            self.ax.scatter(closest_robot_point[0], closest_robot_point[1], c='k', s=10)
            self.ax.scatter(points[0][0], points[0][1], c='b', s=10)
            self.ax.scatter(points[-1][0], points[-1][1], c='r', s=10)
            ...
            self.ax.scatter(bezier_points[:, 0], bezier_points[:, 1], color='k', s=1)
    ...
    def loop(self):
        while not rospy.is_shutdown():
            ...
            self.ax.scatter(robot_points[:, 0], robot_points[:, 1], c = 'b', s=1)
            ...
            self.ax.scatter(obstacle_points[:, 0], obstacle_points[:, 1], c = 'b', s=1)
            plt.draw()
            plt.pause(0.001)
            self.rate.sleep()
